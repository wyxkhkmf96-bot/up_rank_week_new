...
使用方式：
  python update_board_count.py            # 正常跑：累加本日计数
...
规则（当前为天粒度，发布时切回周粒度）：
- 单位为自然日（YYYY-MM-DD）
- 同一日内重复跑不重复加（幂等：以 _meta.last_update_period 锁定）
- 当前榜单的 UP：
    上次记录是"前一日" → count + 1（连续在榜）
    没记录或记录更早 → count = 1（新上榜，因脱榜会被清理，重新出现就当新 UP）
- 当前榜单未出现的 UP：直接从 board_count.json 中删掉（脱榜清理）

发布时切换为周粒度的方法：
1. python update_board_count.py --reset
2. 把下方 GRANULARITY 改为 'week'
3. 第一次按周跑时所有 UP 重新从 1 开始
"""
import json
import os
import sys
...
GRANULARITY = 'day'   # 'day' | 'week'  ← 发布时改为 'week'
