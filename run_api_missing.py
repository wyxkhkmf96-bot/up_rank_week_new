"""W29 LLM 补跑脚本：处理 up_summaries.json 中缺失的 UP"""
import json
import requests
import time
import os
...
LOG_FILE = os.path.join(BASE, 'api_run_resume.log')
...
def load_summaries():
    with open(JSON_OUT, 'r', encoding='utf-8') as f:
        d = json.load(f)
    return d.get('summaries', {}), d.get('_meta', {})
...
def main():
    ...
    for r in arch_rows:
        uid = str(r['UP主ID'])
        if uid in up_id_set:
            if uid not in videos_by_up:
                videos_by_up[uid] = []
            videos_by_up[uid].append(r)

    # 加载现有 summaries
    summaries, meta = load_summaries()
    done_ids = set(summaries.keys())

    # 找出缺失的UP
    missing = [uid for uid in up_ids if uid not in done_ids]
    print(f'新星榜UP总数: {len(up_ids)}')
    print(f'已存在 summaries: {len(done_ids)}')
    print(f'待补跑 UP: {len(missing)}')
    print()
    ...
    # 分类统计
    with_videos = sum(1 for uid in missing if uid in videos_by_up and videos_by_up[uid])
    no_videos = len(missing) - with_videos
    print(f'有稿件: {with_videos}, 无稿件: {no_videos}')
    print()
    ...
    for i, uid in enumerate(missing):
        ...
        if not videos:
            ...
            print(f'  [{i+1}/{len(missing)}] UP{uid} {up_name}: 无稿件')
        else:
            ...
            print(f'  [{i+1}/{len(missing)}] {status} UP{uid} {up_name}: {result[:80]}')

        # 每10个保存一次
        if (i + 1) % 10 == 0 or i + 1 == len(missing):
            save(summaries)
            print(f'  [保存] 已保存 {len(summaries)} 个UP')

        time.sleep(0.4)

    save(summaries)
    print(f'\n完成！共{len(summaries)}个UP（补跑{len(missing)}，错误{error_count}）')
